...
def get_ai_response_for_document_analysis(user_prompt, model_version, temperature, frequency_penalty, presence_penalty,
                                          document_content):
    """专门用于文档分析的AI响应获取函数"""
    try:
        load_dotenv()
        ...
        st.error(f"发生错误：{err}")
        st.write(
            "链接解析失败，可能是由于网络问题或链接本身的合法性问题导致的。请检查链接的合法性，并确保网络连接正常。如果问题仍然存在，建议稍后再试。")
        return '暂时无法获取服务器响应……'
...
def document_analysis():
    ...
    with st.sidebar:
        with st.expander("模型配置"):
            model_version = st.selectbox("选择模型版本", ["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo"])
